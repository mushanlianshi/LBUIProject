...
extension BLTNameSpace where Base == String{
    ...
    public func highLightText(highArray: [String]?, attrs: [NSAttributedString.Key : Any], options: NSString.CompareOptions = .caseInsensitive) -> NSMutableAttributedString? {
        guard let array = highArray else { return nil }
        let attributeString = NSMutableAttributedString.init(string: self.base)
        let nsString = self.base as NSString
        array.forEach { highLightText in
            let range = nsString.range(of: highLightText, options: options)
...
public enum LBPriceFormatterStyle {
    /// 不处理
    case normal
    /// 保留两位小数的格式的
    case keepTwoPoint
...
extension BLTNameSpace where Base == String{
    ...
    public func priceFormatter(_ style: LBPriceFormatterStyle = .deleteLastZero) -> String{
        let number = NSDecimalNumber(string: self.base)
    ...
    /// 两个数字比较
    public func bigThenOther(_ other: String) -> Bool{
        guard let firstValue = Double(self.base) else { return false }
        guard let secondValue = Double(other) else { return false }
        return firstValue > secondValue
    }
    
    public func toDic() -> [String: Any]?{
        guard let jsonData = self.base.data(using: .utf8) else { return nil }
        let jsonObject = try? JSONSerialization.jsonObject(with: jsonData, options: [])
        guard let dic = jsonObject as? [String : Any] else { return nil}
        return dic
    }
    ...
    public func toArray<T>() -> [T]?{
        guard let jsonData = self.base.data(using: .utf8) else { return nil }
        let jsonObject = try? JSONSerialization.jsonObject(with: jsonData, options: [])
        guard let dic = jsonObject as? [T] else { return nil}
        return dic
    }
    
    
    
    func dictionaryToQueryString(_ params: [String: Any]) -> String {
