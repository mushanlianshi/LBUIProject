...
extension BLTNameSpace where Base: UIView{
    ///距离右上角的偏移量
    ///x y 距离右上角的偏移量 如果是isScale就是便宜的比例
    @discardableResult
    public func showRedPointOffsetRightTop(x: CGFloat, y: CGFloat, redColor: UIColor = .red, circleWidth: CGFloat = 5) -> UIView {
        var redView = objc_getAssociatedObject(base, &AssociatedKeys.redPointKey) as? UIView
        if redView == nil{
            redView = UIView()
    ...
    public func hiddenRedPointOffsetRightTop() {
        guard let redView = objc_getAssociatedObject(base, &AssociatedKeys.redPointKey) as? UIView else {
            return
        }
