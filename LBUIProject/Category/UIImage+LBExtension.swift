...
extension BLTNameSpace where Base: UIImage{
    ...
    // 返回一个SF Symbols 里的图片， 默认黑色的图片， 如果要设置颜色，设置imageView的tintColor来改变图标的颜色
    static func imageWithSystemSymbols(name: String, pointSize: CGFloat, weight: UIImage.SymbolWeight = .regular) -> UIImage?{
        let config = UIImage.SymbolConfiguration(pointSize: pointSize, weight: weight)
        let image = UIImage(systemName: name, withConfiguration: config)
        return image
    }
    // 返回一个SF Symbols 里的图片， 可以设置颜色
    static func imageWithSystemColorSymbols(name: String, color: UIColor, pointSize: CGFloat, weight: UIImage.SymbolWeight = .regular) -> UIImage?{
        let config = UIImage.SymbolConfiguration(pointSize: pointSize, weight: weight)
        let image = UIImage(systemName: name, withConfiguration: config)
